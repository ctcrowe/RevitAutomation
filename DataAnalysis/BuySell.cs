            ...
            try
            {
                ...
                for(int ct = 0; ct < 16; ct++)
                {
                    try
                    {
                        ...
                        var finbar = aaplbars.Items[r.Next(aaplbars.Items.Count()) - 1];
                        var compbar = aaplbars.Items.GetComparable();
                        if(DateTime.Compare(finbar.TimeUtc, compbar.TimeUtc) < 0)
                        {
                            Console.WriteLine("Fin Bar Time is : " + finbar.TimeUtc);
                            Console.WriteLine("Comp Bar Time is : " + compbar.TimeUtc);
                            set.FinValue = GetValues(finbar);
                            set.CompValue = GetValues(compbar);
                            set.Increase = (finbar.Open + 1) < compbar.Open;

                            foreach(var b in aaplbars.Items)
                            {
                                if (DateTime.Compare(b.TimeUtc, finbar.TimeUtc) < 0)
                                    set.Values.Add(GetValues(b));
                            }
                            foreach(var b in qqqbars.Items)
                            {
                                if (DateTime.Compare(b.TimeUtc, finbar.TimeUtc) < 0)
                                    set.Values.Add(GetValues(b));

                            }
                            foreach(var b in vtibars.Items)
                            {
                                if (DateTime.Compare(b.TimeUtc, finbar.TimeUtc) < 0)
                                    set.Values.Add(GetValues(b));
                            }
