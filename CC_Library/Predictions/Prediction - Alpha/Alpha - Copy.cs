...
namespace CC_Library.Predictions
{
    internal class Alpha
    {
        ...
        public const int PhraseLength = 140;
        ...
        public static double[] Predict(Datatype dt, string s)
        {
            ...
            GlobalContext gcontext = new GlobalContext(dt, Delegates.WriteNull);
            AlphaMem am = new AlphaMem();
            return a.Forward(s, context, gcontext, am, Delegates.WriteNull);
        }
        public double[] Forward(string s, LocalContext context, GlobalContext gcontext, AlphaMem am, WriteToCMDLine write)
        {
            char[] chars = GetChars(s);
            int runs = Math.Min(PhraseLength, chars.Count());
            double[] ctxt = new double[PhraseLength];
            double[,] loc = new double[PhraseLength, DictSize];
            ...
            ctxt = gcontext.Contextualize(ctxt, am, write);
            return Multiply(loc, ctxt);
        }
        public void Backward(string s, double[] DValues, LocalContext context, GlobalContext gcontext, AlphaMem am, WriteToCMDLine write)
        {
            char[] chars = GetChars(s);
            int runs = Math.Min(PhraseLength, chars.Count());
            var LocDValues = am.DLocation(DValues);
            DValues = am.DGlobalContext(DValues);
            DValues = gcontext.Backward(DValues, am, write);
            context.Backward(DValues, runs, am, write);
            Parallel.For(0, runs, j =>
            {
