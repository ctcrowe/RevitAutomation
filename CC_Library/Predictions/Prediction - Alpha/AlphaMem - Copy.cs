...
namespace CC_Library.Predictions
{
    internal class AlphaMem
    {
        ...
        public List<double[]> GlobalContextOutputs { get; set; }

        public AlphaMem()
        {
            LocationOutputs = new List<double[]>[Alpha.PhraseLength]; // Old = PhraseLength, DictSize
            LocalContextOutputs = new List<double[]>[Alpha.PhraseLength];
            GlobalContextOutputs = new List<double[]>();
        ...
        {
            double[] result = new double[Alpha.PhraseLength];
        ...
        public List<double[]> DLocation(double[] dvalues)
        {
            ...
            for (int i = 0; i < GlobalContextOutputs.Last().Count(); i++)
            {
                double[] res = new double[Alpha.DictSize];
                for (int j = 0; j < res.Count(); j++)
                {
                    res[j] = dvalues[j] * GlobalContextOutputs.Last()[i];
                }
                result.Add(res);
            }
            return result;
