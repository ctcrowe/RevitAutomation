...
{
    internal static class MasterformatInitialize
    {
        public static Dictionary<string, Element>[] InitializeMF(this List<Entry> Entries)
        {
            ...
            foreach (Entry entry in Entries)
            {
                foreach (string result in entry.Values)
                {
                    if (!Datasets[0].ContainsKey(result))
                        Datasets[0].Add(result, Datatype.Masterformat.GetElement(result));
                }
