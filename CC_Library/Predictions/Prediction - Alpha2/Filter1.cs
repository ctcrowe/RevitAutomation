...
namespace CC_Library.Predictions
{
    ...
    internal class AlphaFilter1 : IAlphaFilter
    {
        ...
        internal AlphaFilter1(WriteToCMDLine write)
        {
            ...
            ValueNetwork.Layers.Add(new Layer(40, CharSet.CharCount * (1 + (2 * Radius)), Activation.LRelu, 1e-5, 1e-5));
            ...
            (double[] DValues, double[][][][][] outputs, NetworkMem ValMem, NetworkMem FocMem, WriteToCMDLine write, bool tf = false)
        {
            ...
            //if (tf) outputs[0][0][ValueNetwork.Layers.Count()][1].WriteArray("Value Outputs", write);
            //if (tf) outputs[1][0][AttentionNetwork.Layers.Count()][1].WriteArray("Attention Outputs", write);
            ...
            //if (tf) ContextualDVals.WriteArray("Inv Softmax CDV : ", write);
